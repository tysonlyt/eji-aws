<?php
/**
 * @see \WPML_Sticky_Links::menu_content
 */
...
<div class="wrap">
	...
	<p>
		...
		<input type="submit" name="re_scan" value="<?php echo __( 'Scan', 'wpml-sticky-links' ); ?>" id="alp_re_scan_but" 
															  <?php
																if ( ! $total_posts_pages ) :
																	?>
 disabled="disabled"<?php endif; ?> class="button-secondary action" title="<?php echo __( 'Replace permalinks with sticky links in posts that have not been checked', 'wpml-sticky-links' ); ?>" />
	...
	<p>
		<?php echo __( 'Revert sticky urls to permalinks', 'wpml-sticky-links' ); ?> <input type="button" id="alp_revert_urls" value="<?php echo __( 'Start', 'wpml-sticky-links' ); ?>" class="button-secondary action" 
				   <?php
					if ( ! $total_posts_pages_processed ) :
						?>
			disabled="disabled"<?php endif; ?>
																						 title="<?php echo __( 'Change sticky links back to Wordpress permalinks', 'wpml-sticky-links' ); ?>" />
		<span id="alp_rev_items_left">
		<?php
		if ( $total_posts_pages_processed ) {
			echo $total_posts_pages_processed;
			echo ' ';
			echo __( 'items in queue', 'wpml-sticky-links' ); }
		?>
			</span>
