...
function flrt_plugin_details( $result, $action = null, $args = null )
{
    ...
    $response = $api->sendRequest('PATCH', 'updates', $data );
    ...
    if( isset( $options['plugins'] ) && is_array( $options['plugins'] ) ) {
        foreach ( $options['plugins'] as $single_plugin ) {
            if( $single_plugin === FLRT_PLUGIN_BASENAME ) {
                ...
                flrt_set_transient( FLRT_VERSION_TRANSIENT, '-1', 10800 );
                break;
            }
        }
    }
...
if( ! function_exists('flrt_lowercase_seo_vars' ) ){
    add_filter( 'wpc_seo_var_term_name', 'flrt_lowercase_seo_vars', 10, 3 );
    function flrt_lowercase_seo_vars( $termName, $e_name, $is_wp_entity = false )
    {
        ...
        return ($is_wp_entity) ? $termName : strtolower( $termName );
    }
}
...
if( ! function_exists('flrt_term_rating_stars') ){
    ...
    function flrt_term_rating_stars($html, $link_attributes, $term, $filter)
    {
        ...
        $rating_html = '<div class="flrt-star-rating-checkbox">';

        $i = 1;
        $label_star = '<label class="flrt-star-label %s">' . flrt_rating_star() . '</label>';
        while($i <=5){
            $star_class = '';
            if($i <= $rating){
...
if( ! function_exists('flrt_term_rating_stars_label') ){
    add_filter( 'wpc_filters_label_term_html', 'flrt_term_rating_stars_label', 11, 4 );

    function flrt_term_rating_stars_label($html, $link_attributes, $term, $filter)
    {
        $rating_slugs = array(
            'rated-1',
            'rated-2',
...
{   // Fires several times when WordPress is loading
    ...
    if ( ! $force_check ) {
        $transient = flrt_get_transient( FLRT_VERSION_TRANSIENT );
        if ( $transient !== false ) {
            return $transient;
        }
    }
    ...
    flrt_set_transient( FLRT_VERSION_TRANSIENT, $return, DAY_IN_SECONDS );
...
add_action('admin_action_flrt_duplicate_filter_set', function () {
    if (!wp_verify_nonce($_GET['_wpnonce'], 'flrt_duplicate_filter_set')) {
        wp_die(esc_html__('You do not have sufficient permissions to duplicate the Filter Set.', 'filter-everything'));
    }
    ...
    new DuplicateFilterSet($post_id);
    wp_redirect(admin_url('edit.php?post_type=' . FLRT_FILTERS_SET_POST_TYPE));
    exit;
});

if(!function_exists('flrt_add_raw_title')){
    add_filter('wpc_seo_vars_list', 'flrt_add_raw_title');

    function flrt_add_raw_title($seoVars){
        $insert = ['raw_archive_title' => '{raw_archive_title}'];
        $new_seoVars = [];
        foreach ($seoVars as $key => $value) {
            $new_seoVars[$key] = $value;
...
if(!function_exists('flrt_input_raw_title')){
    add_filter('wpc_seo_entities_and_slugs', 'flrt_archive_title_input');

    function flrt_archive_title_input($seoEntitiesAndSlugs){
        $new_seoEntitiesAndSlugs = [];
        foreach ($seoEntitiesAndSlugs as $fieldIndex => $field){
            $new_seoEntitiesAndSlugs[$fieldIndex] = $field;

            if ($fieldIndex === 'wp_entity') {
                $new_seoEntitiesAndSlugs['wp_entity_raw'] = $field;
                $new_seoEntitiesAndSlugs['wp_entity_raw']['label'] = '{raw_archive_title}';
                $new_seoEntitiesAndSlugs['wp_entity_raw']['slug'] = 'raw_archive_title';
            }
        }
