...
class XmlPrepare
{
    ...
    public function getAllFilterSets()
    {
        ...
        foreach ($filter_sets as $set) {
           ...
           /* if($set['ID'] == 298){
                var_dump($set);
                //var_dump(unserialize($filter_fields['post_content']));
            }*/
            foreach ($filter_fields as $field) {
                ...
                /*if($set['ID'] == 298) {
                    ...
                    var_dump($field);
                    var_dump('__' . $field['post_content']['e_name']);
                }
                var_dump($field['post_content']['e_name']);*/
        ...
        //var_dump($this->filter_sets);
    }


    public function getFilterSet($post_id)
    {
        global $wpdb;
        $results = [];
        $sql = "SELECT ID, post_title, post_content, post_excerpt, post_name
    ...
    public function getPermalinksSettings()
    {
    ...
    public function prepeareSeoRules()
    {
        ...
        foreach ($seo_rules as $rule) {
            ...
            if (mb_strpos($rule['post_title'], $this->partOfUrl) !== false) {
                foreach ($rule['filter_sets'] as $post_id => $filter_set) {
                    ...
                    /*if(($post_id == 298 && $rule['post_title'] == '.../os-{any}/productcat-{any}') || $rule['post_title'] == '.../product-category/{any}/color-{any}'){
                        var_dump($rule['post_title']);
    ...
    public function isAuthorSlug($slug)
    {
        global $wp_rewrite;
    ...
    public function getSlugParts($part, $prefix = false)
    {
        $slug_parts = [];
    ...
    public function createLinksFromArray($link_parts, $level_parse_link)
    {
        $hasEmpty = false;
    ...
    public function remove_three_dots($string)
    {
