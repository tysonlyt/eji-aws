...
class SeoFrontend
{
    ...
    private function replaceVariableData( $content )
    {
        ...
        foreach ( $this->get('allqueriedvalues') as $key => $filter ) {
            ...
            $is_wp_entity = false;
            if ($this->getHumanSeoVarName( $filter ) === '{wp_entity}') $is_wp_entity = true;

            if($is_wp_entity){
                $search[] = '{wp_entity_raw}';
            }
            ...
            if($is_wp_entity){
                $replace[] = apply_filters( 'wpc_seo_var_term_name', $term->name, $entity->getName(), $is_wp_entity);
            }
        }
    ...
    public function openMetaDataBlock()
    {
         ...
         */
        if( ! defined( 'FLRT_DISABLE_CREDENTIALS' ) ) {
