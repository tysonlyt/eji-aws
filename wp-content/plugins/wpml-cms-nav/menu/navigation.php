...
<div class="wrap wpml_cms_navigation">
		...
		<table class="widefat wpml_table" cellspacing="0">
				...
				<td>
					<select name="icl_navigation_page_order" id="icl_navigation_page_order">
						<option value="menu_order"
							<?php
							if ( $cms_navigation_settings['page_order'] == 'menu_order' ) :
								?>
								selected="selected"<?php endif; ?>><?php echo __( 'Menu order', 'wpml-cms-nav' ); ?></option>
						<option value="post_name"
							<?php
							if ( $cms_navigation_settings['page_order'] == 'post_name' ) :
								?>
								selected="selected"<?php endif; ?>><?php echo __( 'Alphabetically', 'wpml-cms-nav' ); ?></option>
						<option value="post_date"
							<?php
							if ( $cms_navigation_settings['page_order'] == 'post_date' ) :
								?>
								selected="selected"<?php endif; ?>><?php echo __( 'Creation time', 'wpml-cms-nav' ); ?></option>
					</select>
				</td>
			</tr>
			<tr valign="top">
				...
				<td>
					<p><label for="icl_navigation_show_cat_menu"><input type="checkbox" id="icl_navigation_show_cat_menu" name="icl_navigation_show_cat_menu" value="1"
								<?php
								if ( $cms_navigation_settings['show_cat_menu'] ) :
									?>
									checked="checked"<?php endif ?> /> <?php _e( 'Show blog posts menu', 'wpml-cms-nav' ); ?></label></p>

					<div id="icl_cat_menu_contents"
						<?php
						if ( ! $cms_navigation_settings['show_cat_menu'] ) :
							?>
						...
						<?php if ( 'page' != get_option( 'show_on_front' ) || ! get_option( 'page_for_posts' ) ) : ?>
							<p>
								...
								<label><?php _e( 'Page order', 'wpml-cms-nav' ); ?><input type="text" name="icl_navigation_cat_menu_page_order" value="<?php echo intval( $cms_navigation_settings['cat_menu_page_order'] ); ?>"
																						  size="3"/></label></p>
						<?php endif; ?>
						<p>
							<?php _e( 'Select what items to display for the blog menu:', 'wpml-cms-nav' ); ?>&nbsp;
							<label><input type="radio" name="icl_blog_menu_contents" value="categories"
									<?php
									if ( $cms_navigation_settings['cat_menu_contents'] == 'categories' ) :
										?>
										checked="checked"<?php endif ?> /><?php _e( 'Categories', 'wpml-cms-nav' ); ?></label>&nbsp;
							<label><input type="radio" name="icl_blog_menu_contents" value="posts"
									<?php
									if ( $cms_navigation_settings['cat_menu_contents'] == 'posts' ) :
										?>
										checked="checked"<?php endif ?> /><?php _e( 'Recent posts', 'wpml-cms-nav' ); ?></label>&nbsp;
							<label><input type="radio" name="icl_blog_menu_contents" value="nothing"
									<?php
									if ( $cms_navigation_settings['cat_menu_contents'] == 'nothing' ) :
										?>
										checked="checked"<?php endif ?> /><?php _e( 'Nothing', 'wpml-cms-nav' ); ?></label>
						</p>
					</div>
				</td>
			</tr>
			<tr valign="top">
				...
				<td valign="top">
					<p style="padding-top:0;margin-top:3px;">
						<label for="icl_navigation_heading_start"><?php echo __( 'Heading start', 'wpml-cms-nav' ); ?> <input type="text" size="6" id="icl_navigation_heading_start" name="icl_navigation_heading_start"
																															  value="<?php echo esc_attr( $cms_navigation_settings['heading_start'] ); ?>"/></label>
						<label for="icl_navigation_heading_end"><?php echo __( 'Heading end', 'wpml-cms-nav' ); ?> <input type="text" size="6" id="icl_navigation_heading_end" name="icl_navigation_heading_end"
																														  value="<?php echo esc_attr( $cms_navigation_settings['heading_end'] ); ?>"/></label>
					</p>
				</td>
			</tr>
			...
			<?php if ( ! defined( 'ICL_DISABLE_CACHE' ) || ! ICL_DISABLE_CACHE ) : ?>
				...
				<tr valign="top">
					...
					<td>
						<p>
							<label for="icl_navigation_caching"><input type="checkbox" id="icl_navigation_caching" name="icl_navigation_caching" value="1"
									<?php
									if ( $cms_navigation_settings['cache'] ) :
										?>
										checked="checked"<?php endif ?> /> <?php echo __( 'Cache navigation elements for super fast performance', 'wpml-cms-nav' ); ?></label>
						</p>
	...
	<table class="widefat" cellspacing="0">
		...
		<tr>
			...
			<td scope="col"><?php printf( __( '%1$s or %2$s, %3$s, %4$s, %5$s and %6$s', 'wpml-cms-nav' ), 'header.php', 'single.php', 'page.php', 'archive.php', 'tag.php', 'search.php' ); ?></td>
		</tr>
		<tr>
