...
class B2bking {
	...
	function __construct() {
		...
		// Fallback theme locations for B2BKing email templates
		add_filter( 'woocommerce_locate_template', array($this, 'b2bking_email_template_fallback'), 10, 3 );
		...
		if (intval(get_option('b2bking_enable_early_access', 1)) === 1){
			...
			if (is_admin()){
				...
				if (get_option('b2bking_early_access_group_rules', 'yes') === 'yes'){
		...
		// Tax toggle shortcode
		add_shortcode('b2bking_tax_toggle', array($this,'b2bking_tax_toggle_shortcode_content'));
		add_action('wp_ajax_b2bking_vat_toggle', array($this, 'b2bking_tax_toggle_ajax'));
		add_filter('option_woocommerce_tax_display_shop', array($this, 'b2b_toggle_tax_display'), 999);
		add_filter('option_woocommerce_tax_display_cart', array($this, 'b2b_toggle_tax_display'), 999);

	}

	function b2bking_tax_toggle_shortcode_content(){
	    if (!WC()->session) return '';
	    
	    // Get user's selection, default to null (no override)
	    $user_setting = WC()->session->get('b2bking_vat_display', null);

	    // Determine effective display: user setting or WooCommerce default
	    $is_incl = $user_setting !== null ? ($user_setting === 'incl') : (get_option('woocommerce_tax_display_shop') === 'incl');
	    ...
	    $texts = apply_filters('b2bking_vat_toggle_texts', [
	        'incl' => $incvat,
	        'excl' => $exvat,
	    ]);
	    ...
	    ob_start();
	    ?>
	    <div class="b2bking-vat-toggle">
	        <style>
	            .b2bking-vat-toggle{display:inline-flex;align-items:center;gap:10px}
	            .b2bking-vat-switch{position:relative;width:44px;height:24px}
	            .b2bking-vat-switch input{opacity:0;width:0;height:0}
	            .b2bking-vat-slider{position:absolute;cursor:pointer;inset:0;background:#999;border-radius:24px;transition:.3s}
	            .b2bking-vat-slider:before{content:"";position:absolute;height:18px;width:18px;left:3px;bottom:3px;background:#fff;border-radius:50%;transition:.3s}
	            .b2bking-vat-switch input:checked+.b2bking-vat-slider{background:#0073aa}
	            .b2bking-vat-switch input:checked+.b2bking-vat-slider:before{transform:translateX(20px)}
	            .b2bking-vat-label{font-size:14px;font-weight:500;color:#333;min-width:70px}
	        </style>
	        <label class="b2bking-vat-switch">
	            <input type="checkbox" id="b2bking-vat-checkbox" <?php checked($is_incl); ?>>
	            <span class="b2bking-vat-slider"></span>
	        </label>
	        <span class="b2bking-vat-label"><?php echo esc_html($is_incl ? $texts['incl'] : $texts['excl']); ?></span>
	    </div>
	    <script>
	    jQuery('#b2bking-vat-checkbox').on('change',function(){
	        jQuery.post('<?php echo esc_url(admin_url('admin-ajax.php')); ?>',{
	            action:'b2bking_vat_toggle',
	            incl:this.checked?1:0,
	            nonce:'<?php echo wp_create_nonce('b2bking_vat_toggle'); ?>'
	        },function(r){if(r.success){jQuery(document.body).trigger('wc_fragment_refresh'); location.reload();}});
	    });
	    </script>
	    <?php
	    return ob_get_clean();
	...
	function b2bking_tax_toggle_ajax() {
	    check_ajax_referer('b2bking_vat_toggle', 'nonce');
	    WC()->session->set('b2bking_vat_display', !empty($_POST['incl']) ? 'incl' : 'excl');
	    wp_send_json_success();
	}

	function b2b_toggle_tax_display($value) {
	    if (WC()->session && ($override = WC()->session->get('b2bking_vat_display'))) {
	        return $override;
	    }
	    return $value;
	}
			...
			if ($user_override === 'manual'){
				...
				foreach ($gateways as $gateway_id => $gateway_value){
					...
					if (!metadata_exists('user', $user_id, 'b2bking_user_payment_method_'.$gateway_id)){
						if (apply_filters('b2bking_payment_shipping_methods_enabled_default', true)){
							$user_access = 1;
						}
					}
	...
	function register_metadata(){
		...
		$array_options = array('b2bking_b2buser','b2bking_customergroup','b2bking_account_approved', 'b2bking_account_type','b2bking_account_parent','b2bking_subaccounts_list');
	...
	// Allow B2BKing email templates to be overridden from theme/woocommerce/emails/ or theme/b2bking/
	// in addition to the documented theme root location
	function b2bking_email_template_fallback( $template, $template_name, $template_path ) {
		// Only act on B2BKing email templates
		if ( strpos( $template_path, B2BKING_DIR ) === false ) {
			return $template;
		}

		// Template was already found in the theme (root override) — leave it alone
		$stylesheet_dir = get_stylesheet_directory();
		$template_dir   = get_template_directory();
		if ( strpos( $template, $stylesheet_dir ) === 0 || strpos( $template, $template_dir ) === 0 ) {
			return $template;
		}

		// Try additional theme subdirectories before falling back to the plugin default
		$found = locate_template( array(
			'woocommerce/emails/' . $template_name,
			'b2bking/' . $template_name,
		) );
	...
	function b2bkingrequestquotecart(){
		...
		if (!is_user_logged_in() || (get_user_meta(get_current_user_id(), 'b2bking_b2buser', true) !== 'yes' && get_option( 'b2bking_plugin_status_setting', 'b2b' ) === 'hybrid')){
			...
			update_post_meta( $conversationid, 'b2bking_conversation_last_reply_time', time() );
			update_post_meta( $conversationid, 'b2bking_logged_out_quote', 'yes');
	...
	function b2bkingactivatelicense(){
	    ...
	    // send activation request — try primary, fallback to VPS proxy if connection fails
	    $endpoints = array(
	        'https://kingsplugins.com/wp-json/licensing/v1/request',
	        'https://apifallback.wpbay.co/wp-json/licensing/v1/request',
	    );
	    
	    $query_string = "?email=".$email."&license=".$key."&requesttype=siteactivation&plugin=BK&website=".$bottom_host_name;
	    ...
	    foreach ($endpoints as $endpoint) {
	        ...
	        curl_setopt_array($curl, [
	            CURLOPT_URL => $endpoint . $query_string,
	            ...
	            CURLOPT_USERAGENT => 'B2BKing/' . (defined('B2BKING_VERSION') ? B2BKING_VERSION : '1.0') . '; WordPress/' . get_bloginfo('version') . '; ' . get_bloginfo('url'),
	            CURLOPT_HTTPHEADER => [
	        ...
	        // Got a response (success or rejection) — use it, don't fallback
	        $response = json_decode($raw_response);
	        break;
	    }
	    
	    // If all endpoints had connection errors
	    if ($response === null && $err) {
	        update_option('b2bking_license_activation_error', array(
	            'error' => $err,
	    ...
	    if ($response === 'success'){
	        ...
	        delete_option('b2bking_license_activation_error');
	    } else {
	        if (empty($response)){
			...
			if ((intval(get_option( 'b2bking_all_products_visible_all_users_setting', 1 )) === 1)){
				...
				*/
				...
				if (!$items_not_manual_visibility_array){
					...
					if (!b2bking()->use_wpml_cache()){
						set_transient('b2bking_not_manual_visibility_array', $items_not_manual_visibility_array, 30 * DAY_IN_SECONDS);
					} else {
						set_transient('b2bking_not_manual_visibility_array'.ICL_LANGUAGE_NAME_EN, $items_not_manual_visibility_array, 30 * DAY_IN_SECONDS);
					}
				}
				...
				if (intval($search_each_variation) === 1){
					...
					if (!$items_not_manual_variations_visibility_array){
						...
						if (!b2bking()->use_wpml_cache()){
							set_transient('b2bking_not_manual_variations_visibility_array', $items_not_manual_variations_visibility_array, 30 * DAY_IN_SECONDS);
						} else {
							set_transient('b2bking_not_manual_variations_visibility_array'.ICL_LANGUAGE_NAME_EN, $items_not_manual_variations_visibility_array, 30 * DAY_IN_SECONDS);
						}
					}
				}
			    ...
			    if (intval($search_each_variation) === 1){
			    	...
			    	if (!$tax_query_prod_ids){
		    			...
		    			set_transient('b2bking_search_tax_query_prod_ids_'.get_current_user_id(), $tax_query_prod_ids->posts, 30 * DAY_IN_SECONDS);
			    ...
			    if (intval($search_each_variation) === 1){
			    	...
			    	if (!$manual_visible_ids){
		    			...
		    			set_transient('b2bking_search_manual_visibility_visible_ids', $manual_visible_ids->posts, 30 * DAY_IN_SECONDS);
	...
	function b2bking_admin_user_search(){
		...
		if (!empty($search_term) && strlen($search_term) >= 3) {
			...
			if(get_option('b2bking_plugin_status_setting', 'b2b') === 'hybrid' && !apply_filters('b2bking_admin_search_show_b2c_users', false)){
				$args['meta_query'] = array(
					array(
						'key' => 'b2bking_b2buser',
						'value' => 'yes',
