...
if (!class_exists('B2bking_New_Customer_Requires_Approval_Email')){
    class B2bking_New_Customer_Requires_Approval_Email extends WC_Email {
        ...
        public function trigger($customer_id, $data = '', $password = '') {
            ...
            if (b2bking()->get_user_application_type($customer_id) === 'agent'){
                $this->subject = esc_html__('New sales agent requires approval', 'b2bking');
                $this->heading = esc_html__('New sales agent requires approval', 'b2bking');
                add_filter('b2bking_requires_approval_email_heading', function($heading, $username){
                    return esc_html__('New sales agent requires approval', 'b2bking');
                }, 10, 2);
            }
            ...
            if ($account_approved === 'no'){
                $final_subject = $this->get_subject();

                if (b2bking()->get_user_application_type($customer_id) === 'agent'){
                    $final_subject = esc_html__('New sales agent requires approval', 'b2bking');
                }

                $this->send( $this->get_recipient(), $final_subject, $this->get_content(), $this->get_headers(), $this->get_attachments() );
            } else {
