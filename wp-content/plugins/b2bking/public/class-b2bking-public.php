...
class B2bking_Public{
				...
				if ( defined( 'WC_PLUGIN_FILE' ) && defined('B2BKINGCORE_DIR')) {
					...
					if ( get_option( 'b2bking_plugin_status_setting', 'b2b' ) !== 'disabled' ){
						...
						if (intval(get_option('b2bking_disable_group_tiered_pricing_setting', 0)) === 0){
							...
							// Bundle integration
							add_filter( 'woocommerce_bundled_item_price', array($this, 'b2bking_individual_pricing_fixed_price_bundle'), 99999, 4 );
						//	add_filter( 'woosb_get_price', array($this, 'b2bking_individual_pricing_fixed_price_bundle'), 99, 4 );
	    ...
	    foreach ($taxonomies as $taxonomy) {
	        ...
	        if (is_array(${'b2bking_all_categories'.$taxonomy})) {
	            ...
	            if ($b2bking_cached_categories_original !== ${'b2bking_all_categories'.$taxonomy}) {
	                ...
	                set_transient('b2bking_cached_categories_taxonomies'.$taxonomy, ${'b2bking_all_categories'.$taxonomy}, YEAR_IN_SECONDS);
	            }
	        }
	    }
	}
	...
	function b2bking_group_rules_apply($order_id, $posted_data = array(), $order = array()){
		...
		$pro_enabled = (get_option('b2bking_early_access_group_rules', 'yes') === 'yes' && class_exists('B2BKing_Group_Rules_Pro'));
		if ($pro_enabled){
	...
	function b2bking_individual_pricing_fixed_price_bundle($price, $product, $discount = 0, $bundled_item = 0){
		...
		if (is_user_logged_in()){
	    	...
	    	$individual_meta_price = get_post_meta($product_id,'b2bking_sale_price_user_'.$user_id, true);
	    	if (empty($individual_meta_price)){
	    		$individual_meta_price = get_post_meta($product_id,'b2bking_sale_price_user_'.$user_name, true);
	    	}
	    	if (empty($individual_meta_price)){
	    		$individual_meta_price = get_post_meta($product_id,'b2bking_regular_price_user_'.$user_id, true);
	    	}
	    	if (empty($individual_meta_price)){
	    		$individual_meta_price = get_post_meta($product_id,'b2bking_regular_price_user_'.$user_name, true);
	    	}
	    	...
	    	if (empty($individual_meta_price)){
				...
				if ($is_b2b_user === 'yes'){
					// Search if there is a specific price set for the user's group
					$b2b_price = get_post_meta($product_id, 'b2bking_sale_product_price_group_'.$currentusergroupidnr, true );
					if (empty($b2b_price)){
						$b2b_price = get_post_meta($product_id, 'b2bking_regular_product_price_group_'.$currentusergroupidnr, true );
					}
	...
	function b2bking_custom_registration_fields_checkout(){
		...
		// show all retrieved fields
			...
			if ($billing_exclusive !== 1){
				...
				echo '<label for="'.esc_attr($labelfor).'">'.trim(wp_kses( $field_label, array( 'a'     => array(
				        'href' => array(), 'target' => array()
				    ) ) )).'&nbsp;';
					if ($required === 'required'){ 
						echo '<span class="required">*</span>'; 
	...
	function b2bking_check_user_approval_on_registration_checkout($order_id) {
		...
		if ($user_approval === 'no'){
			...
			wc_add_notice( apply_filters('b2bking_registration_manual_approval_message', esc_html__('Thank you for registering. Your account requires manual approval. Please wait to be approvedddd.', 'b2bking'), 'success' ));			
			...
			// else it means user is B2B so follow B2B rules
				...
				if ($user_override === 'manual'){
					...
					foreach ( $rates as $rate_id => $rate ) {
						...
						// enabled if metadata empty
						if (!metadata_exists('user', $user_id, 'b2bking_user_shipping_method_'.$rate->method_id)){
							if (apply_filters('b2bking_payment_shipping_methods_enabled_default', true)){
								$user_access = 1;
							}
						}
					...
					foreach ( $rates as $rate_id => $rate ) {
						...
						// enabled if metadata empty
						if (!metadata_exists('post', $currentusergroupidnr, 'b2bking_group_shipping_method_'.$rate->method_id)){
							if (apply_filters('b2bking_payment_shipping_methods_enabled_default', true)){
								$group_access = 1;
							}
						}
	    	...
	    	if (!empty($smpr_rules)){
	    		foreach ($available as $rate_id => $rate){
	    			...
	    			foreach ($smpr_rules as $rule){
	    				...
	    				// Add support for partial matches, e.g. virtual methods like Fedex plugin
	    				$partial_match = false;
	    				$rate_id_start = explode(':', $rate_id)[0];
	    				$rule_rate_id_start = explode(':', $rule_shippingmethod)[0];
	    				if ($rate_id_start === $rule_rate_id_start){
		    				if ($rate_id_start === 'ph_fedex_shipping_zone'){
		    					$partial_match = true;
		    				}
		    			}

	    				if ($rate_id === $rule_shippingmethod || $partial_match){
			...
			if (get_option('b2bking_guest_access_restriction_setting', 'hide_prices') === 'hide_website'){
				if (!is_user_logged_in()) {
					if ( (!is_shop() || apply_filters('b2bking_redirect_shop_hide_site', false)) && !is_account_page() && apply_filters('b2bking_guest_shop_redirect_conditional', true)) {
						...
						wp_redirect( 
						    add_query_arg( 
						        apply_filters('b2bking_redirect_to_path', 'redirect_to'), 
		...
		if ( $error !== 'no') {
			...
			foreach ($csv as $line){
	    		...
	    		if ($id !== 0 && !empty($id)){
	    			...
	    			if ($possible_parent_id !== 0){
	    				$added = WC()->cart->add_to_cart( $possible_parent_id, intval($qty), $id);
	    			} else {
	    				$added = WC()->cart->add_to_cart( $id, intval($qty));
	    			}
	...
	*/
				...
				<div class="b2bking_bulkorder_form_container_content">
					...
					<?php 
						...
						foreach ($list_items as $list_item){
							...
							// Check if product is valid and not trashed - if not, mark for cleanup and skip
							if (!is_a($productobj,'WC_Product_Variation') && !is_a($productobj,'WC_Product')){
								$cleanup_needed = true;
								continue; // Skip this deleted product
							}

							// Also skip products that are in the trash
							if ($productobj->get_status() === 'trash') {
								$cleanup_needed = true;
								continue;
							}
	...
	function b2bking_login_redirect_to( $redirect ) {
	    if ( isset( $_GET[apply_filters('b2bking_redirect_to_path', 'redirect_to')] ) && ! empty( $_GET[apply_filters('b2bking_redirect_to_path', 'redirect_to')] ) ) {
	        $redirect_to = urldecode( $_GET[apply_filters('b2bking_redirect_to_path', 'redirect_to')] );
			...
			if ( get_option( 'b2bking_plugin_status_setting', 'b2b' ) !== 'disabled' ){
				...
				if (!b2bking()->use_wpml_cache()){
					if (!get_transient('b2bking_user_exclude_categories_id_'.$currentuserid)){
						set_transient('b2bking_user_exclude_categories_id_'.$currentuserid, $hiddencategories, YEAR_IN_SECONDS);
					}
				} else {
					if (!get_transient('b2bking_user_exclude_categories_id_'.$currentuserid.ICL_LANGUAGE_NAME_EN)){
						set_transient('b2bking_user_exclude_categories_id_'.$currentuserid.ICL_LANGUAGE_NAME_EN, $hiddencategories, YEAR_IN_SECONDS);
					}
				}
			} else{
				if (!b2bking()->use_wpml_cache()){
						    ...
						    if(!empty($allTheIDs)){
						    	...
						    	if (apply_filters('b2bking_enable_visibility_posts', true)){
			    			    	...
			    			    	if ( ! $all_products_ids){
		    			    			...
		    			    			set_transient('b2bking_all_products_ids', $all_products_ids, 30 * DAY_IN_SECONDS);
			    			    	}
