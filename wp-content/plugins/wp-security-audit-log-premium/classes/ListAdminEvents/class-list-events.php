	...
	<p class="search-box" style="position:relative">
		...
		public function handle_table_actions() {
			...
			if ( ( ( isset( $_REQUEST['action'] ) && 'delete' === $_REQUEST['action'] ) || ( isset( $_REQUEST['action2'] ) && 'delete' === $_REQUEST['action2'] ) ) && Settings_Helper::current_user_can( 'edit' ) ) {
				...
				if ( ! wp_verify_nonce( $nonce, 'bulk-' . $this->_args['plural'] ) ) {
					...
					Metadata_Entity::delete_by_occurrence_ids( $_REQUEST[ $this->table_name ], self::$wsal_db );
				}
			}
			// phpcs:disable
			/* @premium:end */
		...
		public function extra_tablenav( $which ) {
			...
			if ( count( $this->items ) > 0 && count( self::$query_occ ) > 0 ) {
				echo '<input id="wsal-csv-export-button-' . $which . '" class="button" type="button" value="' . esc_html__( 'Export search results to CSV', 'wp-security-audit-log' ) . '" data-query="' . \base64_encode( \serialize( self::$query_occ ) ) . '" data-query-order="' . \htmlspecialchars( json_encode( self::$query_order ), ENT_QUOTES, 'UTF-8' ) . '"  data-wsal-export-csv-nonce="' . \esc_attr( wp_create_nonce( 'wsal-export-csv-nonce' ) ) . '" data-all-records="' . $this->count . '" data-position="' . $which . '"/>';
				...
				<?php } ?>
				<div class="container-pb" id="csv-download-<?php echo $which; ?>">
				<div><?php echo __( 'Do not close this window or exit until the export is complete', 'wp-security-audit-log' ); ?></div>
					<div class="progress-bar__container">
						<div class="progress-bar">
						<span class="progress-bar__text"><?php echo __( 'Generated Successfully!', 'wp-security-audit-log' ); ?></span>
						</div>
					</div>
				</div>
				<?php
