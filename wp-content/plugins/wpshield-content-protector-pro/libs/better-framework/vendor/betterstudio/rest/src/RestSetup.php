 ...
 */
final class RestSetup implements \BetterFrameworkPackage\Core\Module\NeedSetup {
	...
	public static function register( $handler ): bool {
		...
		$classname = $handler instanceof \BetterFrameworkPackage\Core\Rest\RestHandler ? get_class( $handler ) : $handler;
	...
	public static function rest_init() {
		...
		foreach ( static::$routes as $handler ) {
			...
			if ( ! $handler instanceof \BetterFrameworkPackage\Core\Rest\RestHandler ) {
			...
			register_rest_route(
				...
				[
					...
					'allow_batch'         => $handler instanceof \BetterFrameworkPackage\Core\Rest\AllowBatch ? $handler->allow_batch() : [],
