...
(function($) {
    ...
    const Modal = {
        ...
        create: function() {
            console.log('🔨 Creating modal with tabs...');
                ...
                <div id="ucp-product-selector-modal" class="ucp-modal">
                    <div class="ucp-modal-content">
                        ...
                        <div class="ucp-modal-tabs">
                            <button class="ucp-tab-btn active" data-tab="select">Select Products</button>
                            <button class="ucp-tab-btn" data-tab="selected">Selected Products (<span class="ucp-selected-count">0</span>)</button>
                        </div>
                            ...
                            <div class="ucp-tab-content active" data-tab-content="select">
                                <div class="ucp-modal-toolbar">
                                    <div class="ucp-search-container">
                                        <input type="text" id="ucp-product-search" placeholder="Search products">
                                        <button id="ucp-search-btn" class="ucp-btn ucp-btn-sm">Search</button>
                                    </div>
                        ...
                        <div class="ucp-modal-footer">
                            <div class="ucp-selected-info">
                                Selected <span class="ucp-selected-count">0</span> products
                            </div>
    ...
    const Data = {
        ...
        load: function(currentPage = 1, currentCategory = '', currentSearch = '') {
            ...
            if (!Config.ajaxUrl) {
                // console.error('错误: AJAX URL不存在，无法加载产品');
                UI.hideLoading();
    ...
    const UI = {
        ...
        /**
         * 自动保存选择的产品，无需点击"添加"按钮
         * 在创建页面直接保存到隐藏字段，在编辑页面通过AJAX保存
         */
        /**
         * 切换产品选择状态
         * @param {jQuery} $product 产品元素jQuery对象
         */
        ...
        autoSaveSelection: function() {
            try {
                // 详细日志：记录函数调用
                // console.log('UCP DEBUG: autoSaveSelection被调用');
                
                // 如果没有选择产品，不执行任何操作
                if (State.selectedProducts.length === 0) {
                    // console.log('UCP DEBUG: 没有选择产品，不执行保存');
                    return;
                }
                
                // console.log('UCP DEBUG: 自动保存选择的产品');
                
                // 检查页面上是否有隐藏字段 - 不再检查按钮ID
                // console.log('UCP DEBUG: 隐藏字段存在');
                
                // 只要隐藏字段存在，就保存产品ID
                if ($('#ucp-selected-products').length > 0) {
                    // 保存前记录隐藏字段当前值
                    var oldValue = $('#ucp-selected-products').val();
                    // console.log('UCP DEBUG: 保存前隐藏字段值');
                    ...
                    // console.log('UCP DEBUG: 保存后隐藏字段值');
                    // console.log('UCP DEBUG: 保存是否成功');
                    
                    // 给隐藏字段添加更改事件，当表单提交前再次验证
                    $('#ucp-selected-products').closest('form').on('submit', function() {
                        // console.log('UCP DEBUG: 表单提交时，隐藏字段值');
                        return true;
                    });
        ...
        removeSelectedProduct: function(productId) {
            ...
            if (index > -1) {
                ...
                // Update product item in select tab if visible
                const $productItem = $(`.ucp-product-item[data-product-id="${productId}"]`);
                if ($productItem.length) {
                    $productItem.removeClass('selected');
                    $productItem.find(Config.selectors.selectButton).removeClass('selected').text('Select');
                }
    ...
    const Events = {
        ...
        initButtonHandler: function() {
            ...
            $(document).on('click', 'button#ucp-create-select-products, button.ucp-select-products-btn', function(e) {
                ...
                try {
                    // Load products
                    console.log('Preparing to load products...');
                    Data.load(1, '', '');
                } catch (error) {
        ...
        initModalHandlers: function() {
            ...
            $(document).on('click', '.ucp-add-selected-products', function(e) {
                ...
                const pageId = $('#ucp-page-id').val();
                if (!pageId) {
                    Utils.showNotice('Page ID not found', 'error');
                    return;
                }
                ...
                // Send AJAX request to save products
                $.ajax({
                    url: Config.ajaxUrl,
                    type: 'POST',
                    data: {
