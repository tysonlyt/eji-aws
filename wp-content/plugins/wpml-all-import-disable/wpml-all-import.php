	...
	final class WPAI_WPML
	{
								...
								array(
									...
									'auto' => array(
										...
										$this->wpml_addon->add_field('unique_key',
											'Unique Identifier chosen during the parent import',
											'text',
											null,
											'To inform WPML that this new post is translation of another post put the same unique identifier like you did for post in main language.',
											true,
			...
			if ( ! $parentImport->isEmpty())
			{
				...
				if ($parent_post_id)
				{
					...
					if ( $trid )
					{
						...
						if (is_wp_error($tid))
						{
							$logger and call_user_func($logger, '<b>ERROR</b>: ' . $tid->get_error_message());
						}
						else
						{
							$logger and call_user_func($logger, sprintf('- Created `%s` translation for `%s`', $import->options['wpml_addon']['lng'], $parent_post->post_title));
						}
					}
				}
			}
		...
		*/
		public function saved( $post_id, $import, $logger )
		{
			...
			$logger and call_user_func($logger, '<b>TEST</b>');
		}
