...
namespace WPShield\Plugin\ContentProtector\Components\AudioProtector;
...
class AudioProtector extends Component implements Module, Installable, Localization {
	...
	public function operation(): bool {
		...
		add_filter( 'the_content', [ $this, 'remove_audio_download_control' ] );

		return true;
	}

	/**
	 * No download audio control list!
	 *
	 ...
	 * @return string
	 */
	public function remove_audio_download_control( string $content ): string {

		return preg_replace( '/<audio\s/', '<audio controlsList="nodownload"', $content );
	}
