...
/**
 * The "change password" event.
 *
 * @since 3.0.0
 */
class Change_Password_Event extends GA4_Event {


	/** @var string the event ID */
	public const ID = 'change_password';

	/** @var string the event trigger action hook  */
	protected string $trigger_hook = 'woocommerce_save_account_details';
	...
	public function get_form_field_title(): string {
		...
		return __( 'Change Password', 'woocommerce-google-analytics-pro' );
	}
	...
	public function get_form_field_description(): string {
		...
		return __( 'Triggered when a customer changes their password.', 'woocommerce-google-analytics-pro' );
	}
	...
	public function track(): void {
		...
		if ( ! empty( $_POST['password_1'] ) ) {
			...
			$this->record_via_api( [ 'category' => 'My Account' ] );
		}
	}
