...
class Lifecycle extends Framework\Plugin\Lifecycle {
	...
	protected function upgrade_to_3_0_0(): void {
		...
		foreach( $this->get_plugin()->get_tracking_instance()->get_event_tracking_instance()->get_events() as $event ) {
			...
			$event_key = $event::ID . '_event_name';

			if ( ! isset( $settings[ $event_key ] ) ) {
				$settings[ $event_key ] = $event->get_default_name();
			}
		}

		// rename some settings
		if ( ! isset( $settings['track_item_list_views_on'] ) ) {
	 ...
	 * @since 3.0.5
	 */
	protected function upgrade_to_3_0_5() : void {

		// 3.0.0 was missing from $upgrade_versions until 3.0.5, so we need to run it here
