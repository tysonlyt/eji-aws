...
class SettingsController extends AbstractController implements ComponentContract
{
    ...
    {
        try {
            $emailNotificationId = SanitizationHelper::input($request->get_param('emailNotificationId'));
    ...
    public function getSettingItem(WP_REST_Request $request)
    {
        try {
            $emailNotificationId = SanitizationHelper::input($request->get_param('emailNotificationId'));
            $emailNotification = $this->getEmailNotificationDataStore()->read($emailNotificationId);
            $settingId = SanitizationHelper::input($request->get_param('settingId'));
            ...
            try {
                ...
                try {
                    ...
                    throw new SettingNotFoundException($exception->getMessage(), $exception);
                }
            }
